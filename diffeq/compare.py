...
class C8SteerableCNN(torch.nn.Module):
    ...
    def __init__(self, n_classes=10):
        ...
        out_type = nn.FieldType(self.r2_act, 36*[self.r2_act.regular_repr])
        ...
        self.block6 = nn.SequentialModule(
            nn.R2Conv(in_type, out_type, kernel_size=5, padding=0, bias=False),
            nn.InnerBatchNorm(out_type),
            nn.ReLU(out_type, inplace=True)
        )
        # self.pool3 = nn.PointwiseAvgPoolAntialiased(out_type, sigma=0.66, stride=1, padding=0)
        self.pool3=nn.PointwiseAvgPool(out_type, kernel_size=2, stride=2)
...
wd=1.5e-3
drop=[0., 0]
# net=rot_base(32,False, dropout=drop)
t1=time.time()
net=C8SteerableCNN()
t2=time.time()
...
model = torch.nn.DataParallel(net, device_ids=range(torch.cuda.device_count())).cuda()
